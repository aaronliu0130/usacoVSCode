...
public:
	...
	bool equal(const Square& rhs) const{return rhs.equal(sq);}
...
inline bool operator==(const Square& lhs, const Square& rhs){return lhs.equal(rhs);}
inline bool operator==(const Square& lhs, const vector<string>& rhs){return lhs.equal(rhs);}
inline istream& operator>>(istream&is,Square& sq){return sq.read(is);}
